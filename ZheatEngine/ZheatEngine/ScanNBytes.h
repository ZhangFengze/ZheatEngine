...
template<typename T>
class ScanNBytes :public Scan
{
public:
	ScanNBytes(HANDLE processHandle, const ScanOption& option);
...
using Scan1Bytes = ScanNBytes<UINT8>;
using Scan2Bytes = ScanNBytes<UINT16>;
using Scan4Bytes = ScanNBytes<UINT32>;
using Scan8Bytes = ScanNBytes<UINT64>;

template<typename T>
ScanNBytes<T>::ScanNBytes(HANDLE h, const ScanOption & op)
	:Scan(h, op)
{
}

template<typename T>
void ScanNBytes<T>::InitialScan(int t, const QList<QVariant> & param)
{
...
template<typename T>
void ScanNBytes<T>::NextScan(int t, const QList<QVariant>& param)
{
	auto pause = std::make_shared<ProcessPauseManager>(option.pause ? handle : nullptr);
...
template<typename T>
size_t ScanNBytes<T>::NumResults() const
{
	return ScanHelpers::NumResults<T>(regions);
}

template<typename T>
QList<typename ScanNBytes<T>::Result> ScanNBytes<T>::Results(size_t maxCount)
{
	return ScanHelpers::Results<T>(regions, maxCount);
}

template<typename T>
bool ScanNBytes<T>::NeedOldValue(int scanType)
{
...
template<typename T>
std::function<bool(T*)> ScanNBytes<T>::PredNewValue(ScanType type, const QList<QVariant>& parameters)
{
...
template<typename T>
std::function<bool(T*, T*)> ScanNBytes<T>::PredOldAndNewValue(ScanType type, const QList<QVariant>& parameters)
{
